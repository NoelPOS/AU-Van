...
function TimeslotAdmin() {
  ...
  useEffect(() => {
    ...
    const fetchTimes = async () => {
      try {
        const res = await fetch(`/api/auth/timeslot?from=${from}&to=${to}`)
    ...
    <Card className="w-full max-w-2xl mx-auto mt-8 shadow-lg">
      ...
      <CardContent className="grid gap-6">
        ...
        <div className="space-y-2">
          ...
          <Select value={newTime} onValueChange={TIMEhandleValueChange}>
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center space-x-2 rounded-md border p-2">
            ...
            <Input
              ...
              onChange={(e) => setNewNumberOfSeats(Number(e.target.value))}
              className="border-0 p-0"
            />
          </div>
        </div>
